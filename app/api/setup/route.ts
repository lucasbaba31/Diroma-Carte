...
// Protégé par une clé secrète définie dans les variables d'environnement
export async function GET(req: Request) {
  ...
  try {
    ...
    const password = process.env.ADMIN_PASSWORD || "DiRoma2024!";
    ...
    const dishes = [
      ...
      // Pièces
      { id:"dr-v2-p1", categoryId:"dr-cat-18", name:"Pavé de rumsteak Rossini", description:"Foie gras poêlé, sauce Périgueux — garniture au choix : frites ou salade", price:34.90, order:0, badge:"Signature" },
    ...
    for (const d of dishes) {
      const { allergens, badge, ...rest } = d as typeof d & { allergens?: string; badge?: string };
      await prisma.dish.upsert({
        where: { id: d.id },
        update: {},
        create: { ...rest, allergens: allergens ?? "[]", badge: badge ?? null, available: true },
      });
      dishCount++;
    }
    ...
    return NextResponse.json({
      ...
      details: {
        user: email,
        password: process.env.ADMIN_PASSWORD || "DiRoma2024!",
